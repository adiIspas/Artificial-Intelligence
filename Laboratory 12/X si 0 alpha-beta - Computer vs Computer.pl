...
joc_alphabeta(st(_,T,_,MAX)):-
    ...
    joc_alphabeta(st(MAX,T,N,MAX)):-
        ...
        write('A mutat C1(MAX)'), nl, nl, afis_tablou(T1),
    ...
    joc_alphabeta(st(MIN,T,N,MAX)) :-
        ...
        alphabeta(st(MIN,T,N,MAX), -100, 100, st(MAX,T1,N1,MAX), _),
        write('A mutat C2(MIN)'), nl, nl,
        afis_tablou(T1),
...
anunt_castigator(J,MAX):-
          J==MAX,write('A castigat C1(MAX) !'),nl;
          juc_opus(J,J_O),J_O==MAX,
          write('A pierdut C1(MAX)!'),nl;
          % write('Bravo !!');
