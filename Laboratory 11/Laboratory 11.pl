...
joc_minimax(st(_,T,_,MAX)) :-
    	tablou_final(T,C), !, anunt_castigator(C,MAX).
    joc_minimax(st(MAX,T,N,MAX)) :-
    	minimax(st(MAX,T,N,MAX), st(MIN,T1,_,MAX), _),
    	write('Am mutat'), nl, afis_tablou(T1),
    	joc_minimax(st(MIN,T1,N,MAX)).
    joc_minimax(st(MIN,T,N,MAX)) :-
    	juc_opus(MIN,MAX),
    	repeat,
    	write('linie: '), read(L),
    	write('coloana: '), read(C),
    	(membru(L,[1,2,3]), membru(C,[1,2,3]), P is 3*(L-1)+C ;
             write(' L si C trebuie sa apartina {1,2,3}, iar casuta trebuie sa fie disponibila.\n'), P is 0),
    	marcheaza(MIN,T,P,T1), !,
    	afis_tablou(T1),
    	joc_minimax(st(MAX,T1,N,MAX)).
